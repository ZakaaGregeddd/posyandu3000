...
import { getBalitas, addBalita, getKKs } from '@/lib/data/db-service';
import { Balita } from '@/lib/data/types';
import { calculateAge } from '@/lib/utils/health';

export default function BalitaPage() {
  const [balitas, setBalitas] = useState<Balita[]>([]);
  const [kks, setKks] = useState<{ noKk: string }[]>([]);
  ...
  const handleRegisterBalita = (e: React.FormEvent) => {
    ...
    if (!noKk || !nama || !tempatLahir || !tanggalLahir || !namaIbu || !namaAyah) {
    ...
    try {
      const newBalita = addBalita({
        ...
        usiaKehamilanSaatLahirWeeks: usiaKehamilanSaatLahir ? parseInt(usiaKehamilanSaatLahir) : undefined
      });

      setBalitas([...balitas, newBalita]);
  ...
  const paginatedBayiList = bayiList.slice((currentPageBayi - 1) * itemsPerPage, currentPageBayi * itemsPerPage);

  const totalPagesBalita = Math.max(Math.ceil(balitaList.length / itemsPerPage), 1);
  const paginatedBalitaList = balitaList.slice((currentPageBalita - 1) * itemsPerPage, currentPageBalita * itemsPerPage);
  ...
  return (
    <div className="max-w-6xl mx-auto w-full space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h2 className="font-headline text-3xl font-bold text-on-background">Data Balita & Bayi</h2>
          <p className="text-sm text-on-surface-variant mt-1">
            Manajemen rekapitulasi data imunisasi dan grafik perkembangan Balita.
          </p>
        </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-headline text-lg font-bold text-on-background">Data Bayi (0-12 Bulan)</h3>
            <p className="text-xs text-on-surface-variant">{bayiList.length} anggota terdaftar</p>
          </div>
        </div>
          ...
          <CardContent className="p-0">
              ...
              <tbody>
                {paginatedBayiList.length === 0 ? (
                  <tr className="bg-white">
                    <td colSpan={6} className="text-center text-on-surface-variant py-8 border border-outline-variant/10 rounded-xl">
                      Tidak ada data bayi ditemukan
                    </td>
                  </tr>
                ) : (
                  paginatedBayiList.map((baby) => (
                    <tr key={baby.id} className={`bg-white hover:bg-slate-50 transition-colors ${baby.statusHidup === 'Meninggal' ? 'opacity-75' : ''}`}>
                      <td className="px-6 py-4 text-sm font-medium border-y border-l border-outline-variant/10 rounded-l-xl text-on-surface">{baby.noKk}</td>
                      <td className="px-6 py-4 text-sm border-y border-outline-variant/10 text-on-surface">{baby.nik || '-'}</td>
                      <td className="px-6 py-4 text-sm font-bold border-y border-outline-variant/10 text-on-surface">{baby.nama}</td>
                      ...
                      <td className="px-6 py-4 border-y border-outline-variant/10">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${
                          baby.statusHidup === 'Hidup' ? 'bg-teal-50 text-teal-700 border border-teal-200' : 'bg-red-50 text-red-700 border border-red-200'
                        }`}>
                          {baby.statusHidup}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right border-y border-r border-outline-variant/10 rounded-r-xl">
                        <Link href={`/dashboard/balita/${baby.id}`}>
                          <button className="group inline-flex items-center gap-2 text-tertiary hover:bg-secondary-brand/40 px-4 py-2 rounded-lg transition-all whitespace-nowrap cursor-pointer">
                            <span className="text-xs font-bold">Lihat Detail</span>
            ...
            {bayiList.length > itemsPerPage && (
              <div className="flex justify-center items-center gap-6 py-4 text-sm font-medium text-on-surface-variant bg-white/50 rounded-xl border border-outline-variant/10 mt-2">
                <button
                  onClick={() => setCurrentPageBayi(p => Math.max(p - 1, 1))}
                ...
                <span>
                  <span className="font-bold text-on-background">{currentPageBayi}</span> dari <span className="font-bold text-on-background">{totalPagesBayi}</span>
                </span>

                <button
                  onClick={() => setCurrentPageBayi(p => Math.min(p + 1, totalPagesBayi))}
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-headline text-lg font-bold text-on-background">Data Balita (1-5 Tahun)</h3>
            <p className="text-xs text-on-surface-variant">{balitaList.length} anggota terdaftar</p>
          </div>
        </div>
        ...
        <Card className="border border-outline-variant/15 overflow-hidden p-0 bg-transparent shadow-none border-none">
          <CardContent className="p-0">
              ...
              <tbody>
                {paginatedBalitaList.length === 0 ? (
                  ...
                  paginatedBalitaList.map((child) => {
                    const isLocked = calculateAge(child.tanggalLahir).years >= 5;
                    return (
                      <tr key={child.id} className={`bg-white hover:bg-slate-50 transition-colors ${child.statusHidup === 'Meninggal' ? 'opacity-75' : ''}`}>
                        <td className="px-6 py-4 text-sm font-medium border-y border-l border-outline-variant/10 rounded-l-xl text-on-surface">{child.noKk}</td>
                        <td className="px-6 py-4 text-sm border-y border-outline-variant/10 text-on-surface">{child.nik || '-'}</td>
                        <td className="px-6 py-4 text-sm font-bold border-y border-outline-variant/10 text-on-surface">
                          <div className="flex items-center gap-2">
                            <span>{child.nama}</span>
                            {isLocked && (
                              <span className="text-[10px] bg-red-100 text-red-800 font-bold px-2 py-0.5 rounded-full" title="Usia >= 5 tahun, entri data dibekukan">
                                Terkunci
                              </span>
                            )}
                          </div>
                        ...
                        <td className="px-6 py-4 border-y border-outline-variant/10">
                          <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${
                            child.statusHidup === 'Hidup' ? 'bg-teal-50 text-teal-700 border border-teal-200' : 'bg-red-50 text-red-700 border border-red-200'
                          }`}>
                            {child.statusHidup}
                          </span>
                        </td>
            ...
            {balitaList.length > itemsPerPage && (
              <div className="flex justify-center items-center gap-6 py-4 text-sm font-medium text-on-surface-variant bg-white/50 rounded-xl border border-outline-variant/10 mt-2">
                <button
                  onClick={() => setCurrentPageBalita(p => Math.max(p - 1, 1))}
                ...
                <span>
                  <span className="font-bold text-on-background">{currentPageBalita}</span> dari <span className="font-bold text-on-background">{totalPagesBalita}</span>
                </span>

                <button
                  onClick={() => setCurrentPageBalita(p => Math.min(p + 1, totalPagesBalita))}
      ...
      <Dialog isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <form onSubmit={handleRegisterBalita} className="flex flex-col max-h-[85vh] overflow-hidden">
          <DialogHeader>
            <DialogTitle>Tambah Data Balita / Bayi</DialogTitle>
            <DialogDescription>
              Masukkan informasi detail balita baru. Pastikan nomor KK telah terdaftar di sistem.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                >
                  ...
                  {kks.map(k => (
                    <option key={k.noKk} value={k.noKk}>{k.noKk}</option>
                  ))}
                </select>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  value={nik}
                  onChange={(e) => {
                    const clean = e.target.value.replace(/[^0-9]/g, '').substring(0, 16);
                    setNik(clean);
                  }}
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <select
                  id="modal_cara_lahir"
                  value={caraLahir}
                  onChange={(e) => setCaraLahir(e.target.value as 'SC' | 'Normal' | '')}
                  className="w-full rounded-lg border border-outline-variant bg-white px-3 py-2 text-sm text-on-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-tertiary focus-visible:ring-offset-2 transition-all"
                >
              ...
              <div className="space-y-1.5">
                <Label htmlFor="modal_usia_kehamilan">Usia Kehamilan (Weeks - Opsional)</Label>
                <Input
