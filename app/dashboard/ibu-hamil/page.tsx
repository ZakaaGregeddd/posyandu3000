...
import { getIbuHamils, addIbuHamil, getKKs } from '@/lib/data/db-service';
import { IbuHamil } from '@/lib/data/types';
...
export default function IbuHamilPage() {
  ...
  const handleRegisterBumil = (e: React.FormEvent) => {
    e.preventDefault();
    setFormError('');
  ...
  const totalPages = Math.max(Math.ceil(filteredBumils.length / itemsPerPage), 1);
  const paginatedBumils = filteredBumils.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  ...
  return (
    <div className="max-w-6xl mx-auto w-full space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h2 className="font-headline text-3xl font-bold text-on-background">Manajemen Ibu Hamil</h2>
          <p className="text-sm text-on-surface-variant mt-1">
            Pantauan HPHT, taksiran kelahiran (HPL), usia kehamilan, dan rujukan melahirkan otomatis.
          </p>
        </div>
          ...
          <table className="w-full border-separate border-spacing-y-2">
            <thead>
              <tr className="text-left text-xs font-bold uppercase tracking-wider text-on-surface-variant">
                <th className="px-6 py-4 bg-secondary-container rounded-l-xl">Status Kelahiran</th>
                <th className="px-6 py-4 bg-secondary-container">No. KK / NIK</th>
                <th className="px-6 py-4 bg-secondary-container">Nama Anggota</th>
                <th className="px-6 py-4 bg-secondary-container">Usia Ibu</th>
                <th className="px-6 py-4 bg-secondary-container">Usia Kandungan</th>
            ...
            <tbody>
              {paginatedBumils.length === 0 ? (
                <tr className="bg-white">
                  <td colSpan={7} className="text-center text-on-surface-variant py-8 border border-outline-variant/10 rounded-xl">
                    Tidak ada data ibu hamil ditemukan
                  </td>
                </tr>
              ) : (
                paginatedBumils.map((bumil) => (
                  <tr key={bumil.id} className={`bg-white hover:bg-slate-50 transition-colors ${bumil.statusHidup === 'Meninggal' ? 'opacity-75' : ''}`}>
                    <td className="px-6 py-4 border-y border-l border-outline-variant/10 rounded-l-xl">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                        ...
                        {bumil.postBirthRecord ? 'Sudah Lahir' : 'Belum Lahir'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm border-y border-outline-variant/10 text-on-surface">
                      <div className="flex flex-col">
                        <span className="font-medium text-on-surface-variant">{bumil.noKk}</span>
                        <span className="text-xs text-outline">{bumil.nik}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm font-bold border-y border-outline-variant/10 text-on-surface">{bumil.nama}</td>
                    <td className="px-6 py-4 text-sm border-y border-outline-variant/10 text-on-surface">{calculateAge(bumil.tanggalLahir).text}</td>
                    <td className="px-6 py-4 text-sm border-y border-outline-variant/10 text-on-surface">
                      {bumil.postBirthRecord ? (
                        <span className="text-xs text-on-surface-variant italic">Melahirkan pada {new Date(bumil.postBirthRecord.tanggalLahir).toLocaleDateString('id-ID')}</span>
                      ) : (
                    ...
                    <td className="px-6 py-4 border-y border-outline-variant/10">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${
                        bumil.statusHidup === 'Hidup' ? 'bg-teal-50 text-teal-700 border border-teal-200' : 'bg-red-50 text-red-700 border border-red-200'
                      }`}>
                        {bumil.statusHidup}
                      </span>
                    </td>
      ...
      <Dialog isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <form onSubmit={handleRegisterBumil} className="flex flex-col max-h-[85vh] overflow-hidden">
          <DialogHeader>
            <DialogTitle>Tambah Data Ibu Hamil</DialogTitle>
            <DialogDescription>
              Masukkan identitas ibu hamil baru beserta Hari Pertama Haid Terakhir (HPHT).
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2 space-y-1.5">
                <Label htmlFor="modal_hpht">Hari Pertama Haid Terakhir (HPHT)</Label>
                <Input
                  id="modal_hpht"
                  type="date"
                  value={hpht}
